...
//                dismissKeyboard()
    ...
    @IBAction func loginClicked(_ sender: UIButton) {
        ...
        AuthService.signIn(controller: self, email: email, password: password) { (user) in
            ...
            UserService.fetchUsers(forUID: user.uid) { (user) in
                if let user = user {
...
extension LoginViewController{
    ...
    func errorAlertViewController()
    {
        let refreshAlert = UIAlertController(title: "Error", message: "User does not exist", preferredStyle: UIAlertControllerStyle.alert)
